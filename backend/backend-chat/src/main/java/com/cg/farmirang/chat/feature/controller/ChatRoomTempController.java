...
@Slf4j
@Controller
@Tag(name = "3. 판잣집 MVC 테스트용", description = "서버 생길 때까지 판자집")
@RequiredArgsConstructor
@RequestMapping("/chats")
public class ChatRoomTempController {
    ...
    public String room(Model model, @RequestParam Integer memberId) {
    ...
    public String createRoom(@RequestParam("memberId") Integer memberId, Model model,@RequestParam("myMemberId") Integer myMemberId) {
        ChatRoomCreateRequestDto request=ChatRoomCreateRequestDto.builder().memberId(memberId).build();
        String chatRoomId = chatRoomService.createChatRoom(myMemberId, request);
    ...
    public String selectChatRoom(@RequestParam Integer memberId,@PathVariable("chatRoomId") String chatRoomId, Model model) {
